                    ...
                    foreach (ParticleSystem ps in particles)
                    {
                        ...
                        if (psr.mesh)
                        {
                            if (psr.renderMode == ParticleSystemRenderMode.Mesh)
                            {
                                ...
                                {
                                    sw.WriteLine(string.Format("illegal vertex count,{0},{1},{2}", s,
                                        AssetDatabase.GetAssetPath(psr.mesh), psr.mesh.vertexCount));
                                }

                                //发射粒子最大数量不能超过5
                                if (ps.main.maxParticles > 5)
                                {
                                    sw.WriteLine(string.Format("illegal max particles,{0},{1},{2}", s, ps.name,
                                ...
                                {
                                    sw.WriteLine(string.Format("illegal mesh ReadWrite,{0},{1}", s,
                                        AssetDatabase.GetAssetPath(psr.mesh)));
                                }
                            }
                            else
                            {
