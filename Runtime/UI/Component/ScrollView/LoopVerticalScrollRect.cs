    ...
    public class LoopVerticalScrollRect : LoopScrollRect
    {
        ...
        protected override bool UpdateItems(Bounds viewBounds, Bounds contentBounds)
        {
            ...
            if (viewBounds.min.y > contentBounds.min.y + threshold + m_ContentBottomPadding)
            {
                ...
                while (size > 0 && viewBounds.min.y > contentBounds.min.y + threshold + totalSize + m_ContentBottomPadding)
                {
            ...
            if (viewBounds.max.y < contentBounds.max.y - threshold - m_ContentTopPadding)
            {
                ...
                while (size > 0 && viewBounds.max.y < contentBounds.max.y - threshold - totalSize - m_ContentTopPadding)
                {
            ...
            if (viewBounds.min.y < contentBounds.min.y + m_ContentBottomPadding)
            {
                ...
                while (size > 0 && viewBounds.min.y < contentBounds.min.y - totalSize + m_ContentBottomPadding)
                {
                    size = NewItemAtEnd();
                    totalSize += size;
                }
            ...
            if (viewBounds.max.y > contentBounds.max.y- m_ContentTopPadding)
            {
                ...
                while (size > 0 && viewBounds.max.y > contentBounds.max.y + totalSize- m_ContentTopPadding)
                {
                    size = NewItemAtStart();
                    totalSize += size;
                }
