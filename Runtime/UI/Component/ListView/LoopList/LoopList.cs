...
namespace ScrollView
{
    ...
    public class LoopList : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        private LoopListItemPoolBase _pool;

        [SerializeField] private ItemPrefabConfData mItemPrefabData;
//        [SerializeField] List<ItemPrefabConfData> mItemPrefabDataList = new List<ItemPrefabConfData>();
        ...
        List<LoopListItemView> mItemList = new List<LoopListItemView>();
        ...
        System.Func<LoopList, int, LoopListItemView> mOnGetItemByIndex;
        ...
        public System.Action<LoopList, LoopListItemView> mOnSnapItemFinished = null;
        public System.Action<LoopList, LoopListItemView> mOnSnapNearestChanged = null;
        ...
        public ItemPrefabConfData ItemPrefabData
        {
            get { return mItemPrefabData; }
        }

        public List<LoopListItemView> ItemList
        {
        ...
        /*
        InitListView method is to initiate the LoopList component. There are 3 parameters:
        ...
        */
        public void InitListView(LoopListItemPoolBase inPool, int itemTotalCount,
            System.Func<LoopList, int, LoopListItemView> onGetItemByIndex,
            LoopListViewInitParam initParam = null)
        {
            ...
            {
                Debug.LogError("LoopList.InitListView method can be called only once.");
            }
        ...
        */
        ...
        public LoopListItemView GetShownItemByItemIndex(int itemIndex)
        {
        ...
        public LoopListItemView GetShownItemNearestItemIndex(int itemIndex)
        {
            int count = mItemList.Count;
            if (count == 0)
            {
        ...
        /*
         All visible items is stored in a List<LoopListItemView> , which is named mItemList;
        ...
        */
        public LoopListItemView GetShownItemByIndex(int index)
        {
            int count = mItemList.Count;
        ...
        public LoopListItemView GetShownItemByIndexWithoutCheck(int index)
        {
            return mItemList[index];
        }

        public int GetIndexInShownItemList(LoopListItemView item)
        {
            if (item == null)
            {
                return -1;
        ...
        public void DoActionForEachShownItem(System.Action<LoopListItemView, object> action, object param)
        {
            if (action == null)
            {
                return;
        ...
        public LoopListItemView NewListViewItem(out GameObject outGo)
        {
            // ItemPool pool = null;
            ...
            LoopListItemView item = _pool.GetItem(out outGo);
            RectTransform rf = outGo.transform as RectTransform;
        ...
        /*
        For a vertical scrollrect, when a visible item’s height changed at runtime, then this method should be called to let the LoopList component reposition all visible items’ position.
        For a horizontal scrollrect, when a visible item’s width changed at runtime, then this method should be called to let the LoopList component reposition all visible items’ position.
        */
        public void OnItemSizeChanged(int itemIndex)
        {
            LoopListItemView item = GetShownItemByItemIndex(itemIndex);
            if (item == null)
            {
                return;
            }
        ...
        */
        public void RefreshItemByItemIndex(int itemIndex)
        {
            ...
            LoopListItemView curItem = mItemList[i];
            Vector3 pos = curItem.CachedRectTransform.anchoredPosition3D;
            RecycleItemTmp(curItem);
            LoopListItemView newItem = GetNewItemByIndex(itemIndex);
            if (newItem == null)
            {
        ...
        */
        ...
        public void RefreshAllShownItemWithFirstIndex(int firstItemIndex)
        {
            ...
            LoopListItemView firstItem = mItemList[0];
            Vector3 pos = firstItem.CachedRectTransform.anchoredPosition3D;
            RecycleAllItem();
            for (int i = 0; i < count; ++i)
            {
                int curIndex = firstItemIndex + i;
                LoopListItemView newItem = GetNewItemByIndex(curIndex);
                if (newItem == null)
                {
                    break;
                }
        ...
        {
            RecycleAllItem();
            LoopListItemView newItem = GetNewItemByIndex(firstItemIndex);
            if (newItem == null)
            {
                return;
            }
        ...
        void RecycleItemTmp(LoopListItemView item)
        {
            if (item == null)
            {
                return;
        ...
        void ClearAllTmpRecycledItem()
        {
            ...
            // }
            _pool.ClearTmpRecycledItem();
        }


        void RecycleAllItem()
        {
            foreach (LoopListItemView item in mItemList)
            {
                RecycleItemTmp(item);
            }
        ...
        void InitItemPool()
        {
            ...
            //     AdjustAnchor(rtf);
            //     AdjustPivot(rtf);
            //     LoopListItemView tItem = data.mItemPrefab.GetComponent<LoopListItemView>();
            //     if (tItem == null)
            //     {
            //         data.mItemPrefab.AddComponent<LoopListItemView>();
            //     }
            //
            ...
            // }
            var data = mItemPrefabData;
            RectTransform rtf = mItemPrefabData.mItemPrefab.GetComponent<RectTransform>();
            AdjustAnchor(rtf);
            AdjustPivot(rtf);
            _pool.Init(data.mItemPrefab, data.mPadding, data.mStartPosOffset, data.mInitCreateCount,
                mContainerTrans);
        }
        ...
        LoopListItemView GetNewItemByIndex(int index)
        {
            ...
            LoopListItemView newItem = mOnGetItemByIndex(this, index);
            if (newItem == null)
            {
                return null;
            }
        ...
        public Vector3 GetItemCornerPosInViewPort(LoopListItemView item,
        ...
        void AdjustPanelPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                LoopListItemView tViewItem0 = mItemList[0];
                ...
                LoopListItemView tViewItem1 = mItemList[mItemList.Count - 1];
        ...
        void UpdateSnapVertical(bool immediate = false, bool forceSendEvent = false)
        {
            ...
            if (immediate || Mathf.Abs(mCurSnapData.mTargetSnapVal - mCurSnapData.mCurSnapVal) < mSnapFinishThreshold)
            {
                ...
                {
                    LoopListItemView targetItem = GetShownItemByItemIndex(mCurSnapNearestItemIndex);
                    if (targetItem != null)
                    {
        ...
        void UpdateCurSnapData()
        {
            ...
            if (mCurSnapData.mSnapStatus == SnapStatus.SnapMoving)
            {
                if (mCurSnapData.mIsForceSnapTo)
                {
                    ...
                    {
                        LoopListItemView targetItem = GetShownItemNearestItemIndex(mCurSnapData.mSnapTargetIndex);
                        if (targetItem == null)
                        {
                            mCurSnapData.Clear();
                            return;
            ...
            {
                LoopListItemView nearestItem = GetShownItemByItemIndex(mCurSnapNearestItemIndex);
                if (nearestItem == null)
                {
                    return;
                }
        ...
        //moveMaxAbsVec param is the max abs snap move speed, if the value <= 0 then LoopList would use SnapMoveDefaultMaxAbsVec
            ...
            float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                if (itemListCount == 0)
                {
                    ...
                    LoopListItemView newItem = GetNewItemByIndex(index);
                    if (newItem == null)
                    {
                        return false;
                    }
                ...
                if (mViewPortRectLocalCorners[0].y - downPos1.y < distanceForNew1)
                {
                    ...
                    {
                        LoopListItemView newItem = GetNewItemByIndex(nIndex);
                        if (newItem == null)
                        {
                            mCurReadyMaxItemIndex = tViewItem1.ItemIndex;
                            mNeedCheckNextMaxItem = false;
        ...
        void CheckIfNeedUpdataItemPos()
        {
            ...
            {
                LoopListItemView firstItem = mItemList[0];
                LoopListItemView lastItem = mItemList[mItemList.Count - 1];
        ...
        void UpdateAllShownItemsPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    LoopListItemView item = mItemList[i];
