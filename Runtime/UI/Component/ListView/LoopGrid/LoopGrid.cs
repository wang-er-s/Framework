...
namespace ScrollView
{
    ...
    public class LoopGrid : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        private LoopGridItemPoolBase _pool;
        ...
        [SerializeField] private GridViewItemPrefabConfData mItemPrefabData;
        ...
        Vector2 mEndPadding;
        System.Func<LoopGrid, int, int, int, LoopGridItemView> mOnGetItemByRowColumn;
        ...
        public System.Action<LoopGrid, LoopGridItemView> mOnSnapItemFinished = null;
        ...
        public GridViewItemPrefabConfData ItemPrefabData
        {
        ...
        */
        public void InitGridView(LoopGridItemPoolBase inPool, int itemTotalCount,
            System.Func<LoopGrid, int, int, int, LoopGridItemView> onGetItemByRowColumn,
        ...
        */
        ...
        public LoopGridItemView NewListViewItem(out GameObject outGo)
        {
            ...
            GameObject go;
            LoopGridItemView item = _pool.GetItem(out go);
            outGo = go;
            RectTransform rf = go.transform as RectTransform;
        ...
        */
        public void RefreshItemByRowColumn(int row, int column)
        {
            ...
            if (mGridFixedType == GridFixedType.ColumnCountFixed)
            {
                ...
                LoopGridItemView cur = group.GetItemByColumn(column);
                if (cur == null)
                {
                    return;
                }

                LoopGridItemView @new = GetNewItemByRowColumn(row, column);
                if (@new == null)
                {
                    return;
                }
                ...
                group.ReplaceItem(cur, @new);
                RecycleItemTmp(cur);
                @new.CachedRectTransform.anchoredPosition3D = pos;
                ClearAllTmpRecycledItem();
            }
            else
            {
                ...
                LoopGridItemView cur = group.GetItemByRow(row);
                if (cur == null)
                {
                    return;
                }
        ...
        */
        ...
        public LoopGridItemView GetShownItemByItemIndex(int itemIndex)
        {
            if (itemIndex < 0 || itemIndex >= ItemTotalCount)
            {
                return null;
        ...
        public LoopGridItemView GetShownItemByRowColumn(int row, int column)
        {
            if (mItemGroupList.Count == 0)
            {
                return null;
        ...
        public void UpdateItemSize()
        {
            ...
            GameObject obj = mItemPrefabData.mItemPrefab;
            if (obj == null)
            {
                return;
            }
        ...
        void RecycleItemGroupTmp(GridItemGroup group)
        {
            ...
            {
                LoopGridItemView item = group.RemoveFirst();
                RecycleItemTmp(item);
            }
        ...
        void RecycleItemTmp(LoopGridItemView item)
        {
            if (item == null)
            {
                return;
        ...
        void InitItemPool()
        {
            ...
            //     LoopGridItemView tItem = data.mItemPrefab.GetComponent<LoopGridItemView>();
            //     if (tItem == null)
            //     {
            //         data.mItemPrefab.AddComponent<LoopGridItemView>();
            //     }
            //
            ...
            RectTransform rtf = data.mItemPrefab.GetComponent<RectTransform>();
            AdjustItemAnchorAndPivot(rtf);
            _pool.Init(data.mItemPrefab, 0, 0, data.mInitCreateCount, mContainerTrans);
        }


        LoopGridItemView GetNewItemByRowColumn(int row, int column)
        {
            ...
            LoopGridItemView @new = mOnGetItemByRowColumn(this, itemIndex, row, column);
            if (@new == null)
            {
                return null;
            }
            ...
            @new.ItemCreatedCheckFrameCount = mListUpdateCheckFrameCount;
            return @new;
        }
        ...
        void UpdateRowItemGroupForRecycleAndNew(GridItemGroup group)
        {
            ...
            if (group.First == null)
            {
                LoopGridItemView item = GetNewItemByRowColumn(row, minColumn);
                if (item == null)
                {
                    return;
                }
            ...
            {
                LoopGridItemView item = GetNewItemByRowColumn(row, group.First.Column - 1);
                if (item == null)
                {
                    break;
                }
            ...
            {
                LoopGridItemView item = GetNewItemByRowColumn(row, group.Last.Column + 1);
                if (item == null)
                {
                    break;
                }
        ...
        void UpdateColumnItemGroupForRecycleAndNew(GridItemGroup group)
        {
            ...
            if (group.First == null)
            {
                LoopGridItemView item = GetNewItemByRowColumn(minRow, column);
                if (item == null)
                {
                    return;
                }
            ...
            {
                LoopGridItemView item = GetNewItemByRowColumn(group.First.Row - 1, column);
                if (item == null)
                {
                    break;
                }
            ...
            {
                LoopGridItemView item = GetNewItemByRowColumn(group.Last.Row + 1, column);
                if (item == null)
                {
                    break;
                }
        ...
        void UpdateSnapMove(bool immediate = false, bool forceSendEvent = false)
        {
            ...
            if (immediate || Mathf.Abs(mCurSnapData.mTargetSnapVal - mCurSnapData.mCurSnapVal) < mSnapFinishThreshold)
            {
                ...
                {
                    LoopGridItemView target = GetShownItemByRowColumn(mCurSnapNearestItemRowColumn.mRow,
                        mCurSnapNearestItemRowColumn.mColumn);
                    if (target != null)
                    {
                        mOnSnapItemFinished(this, target);
                    }
                }
            }
            else
        ...
        void UpdateCurSnapData()
        {
            ...
            {
                LoopGridItemView nearest = GetShownItemByRowColumn(mCurSnapNearestItemRowColumn.mRow,
                    mCurSnapNearestItemRowColumn.mColumn);
                if (nearest == null)
                {
                    return;
            ...
            if (mCurSnapData.mSnapStatus == SnapStatus.TargetHasSet)
            {
                ...
                FillCurSnapData(target.Row, target.Column);
