    ...
    internal sealed class Fsm<T> : FsmBase, IFsm<T> where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new Exception(
                        $"FSM '{new TypeNamePair(typeof(T), name).ToString()}' state '{stateType}' is already exist.");
                }
        ...
        internal void ChangeState(Type stateType)
        {
            ...
            {
                throw new Exception(
                    $"FSM '{new TypeNamePair(typeof(T), Name).ToString()}' can not change state to '{stateType.FullName}' which is not exist.");
            }
