...
namespace AD
{
    ...
    public class AssetBundleParser
    {
        ...
        public AssetBundleParser(string relativePath, byte[] bytes, Action<AssetBundle> callback = null)
        {
            ...
            switch (Mode)
            {
                case CAssetBundleParserMode.Async:
                    ...
                    ResourceModule.Ins.StartCoroutine(WaitCreateAssetBundle(CreateRequest));
                    break;
        ...
        private void OnFinish(AssetBundle bundle)
        {
            ...
            if (Application.isEditor && Debug.isDebugBuild)
            {
                ...
                {
                    Debugger.Warning($"[KAssetBundleParser] Parse Too long time: {RelativePath},  used time: {useTime}");
                }
            }
        }
        ...
        private IEnumerator WaitCreateAssetBundle(AssetBundleCreateRequest req)
        {
            ...
            if (Application.isEditor)
            {
                ...
                {
                    Debugger.Warning($"[CAssetBundlerParser]{RelativePath} 解压/读取Asset太久了! 花了{Time.time - startTime}秒, 超过 {timeout}秒" );
                }
            }
