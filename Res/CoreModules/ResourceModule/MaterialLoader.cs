...
namespace AD
{
    ...
    public class MaterialLoader : AbstractResourceLoader
    {
        ...
        {
            base.Init(url, args);
            ResourceModule.Ins.StartCoroutine(CoLoadSerializeMaterial());
        }
        ...
        private IEnumerator CoLoadSerializeMaterial()
        {
            ...
            var sMat = matLoadBridge.ResultObject as SerializeMaterial;
            Debugger.Assert(sMat);

            Desc = sMat.ShaderName;

            Debugger.Assert(Mat == null);
            yield return ResourceModule.Ins.StartCoroutine(CoGenerateMaterial(Url, sMat));
            Debugger.Assert(Mat);

            matLoadBridge.Release(); //不需要它了

            if (Application.isEditor)
                ResourceLoadedAssetDebugger.Create("Material", Url, Mat);
            OnFinish(Mat);
        }
        ...
        protected override void OnFinish(object resultObj)
        {
            if (resultObj == null)
            {
                Debugger.Error("[MaterialLoader:OnFinishe] Null Material: {0}", Url);
            }
        ...
        private string ParseMaterialStr(string materialTextureStr, out Vector2 tiling, out Vector2 offset)
        {
            ...
            Debugger.Assert(textureArr.Length > 0);
        ...
        // 加载材质的图片, 协程等待
        private IEnumerator CoGenerateMaterial(string matPath, SerializeMaterial sMat)
        {
            // 纹理全部加载完成后到这里
            //if (!CachedMaterials.TryGetValue(matPath, out mat))
            {
                ...
                if (shader == null)
                {
                    ...
                    Debugger.Warning("无法加载Shader资源: {0}, 使用Shaders.Find代替", sMat.ShaderName);
                    if (shader == null)
                    {
                        Debugger.Warning("找不到Shader: {0}, 使用Diffuse临时代替", sMat.ShaderName);
                        shader = KTool.FindShader("Diffuse");
                    }
                }
                Debugger.Assert(shader);
                ...
                foreach (SerializeMaterialProperty shaderProp in sMat.Props)
                {
                    switch (shaderProp.Type)
                    {
                        case SerializeMaterialProperty.ShaderType.Texture:
                            Vector2 tiling;
                            Vector2 offset;
                            ...
                            if (tex == null)
                            {
                                Debugger.Error("找不到纹理: {0}", texturePath);
                            }
                            else
                            {
                            ...
                            break;
                        case SerializeMaterialProperty.ShaderType.Color:
                            _SetMatColor(Mat, shaderProp.PropName, shaderProp.PropValue);
                            break;
                        case SerializeMaterialProperty.ShaderType.Range:
                            _SetMatRange(Mat, shaderProp.PropName, shaderProp.PropValue);
                            break;
                        case SerializeMaterialProperty.ShaderType.Vector:
                            _SetMatVector(Mat, shaderProp.PropName, shaderProp.PropValue);
                            break;
                        case SerializeMaterialProperty.ShaderType.RenderTexture:
        ...
        private void _SetMatRange(Material mat, string propName, string propValue)
        {
            if (mat.HasProperty(propName))
            {
                ...
                Debugger.Error("[_SetMatRange]Cannot find shader property: {0}", propName);
        }
        ...
        private void _SetMatColor(Material mat, string colorPropName, string _colorStr) // 设置材质Shader的颜色
        {
            if (mat.HasProperty(colorPropName))
            {
                ...
                if (mat.HasProperty(colorPropName))
                    ...
                    Debugger.Error("[_SetMatColor]Cannot find shader property: {0}", colorPropName);
            }
        }
