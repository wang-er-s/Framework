...
namespace Framework
{
	public abstract class BundleHolder
	{
		...
		private BundleMgr mgr;

		private int count;
		private int retryTimes;
		private float noRefTime; //没有引用的时间

		private List<AssetCallback> cbs = new List<AssetCallback>();
		private bool isLoading;
		private BundleLoadTask loadTask;
		private int waitDependNum;
		private Dictionary<ulong, string> waitDependCbIds;
		...
		private void StartLoad()
		{
			...
			retryTimes = 0;
			isLoading = true;
			waitDependNum = info.depends?.Length ?? 0;
			if (waitDependNum > 0)
			{
		...
		{
			if (IsLoaded() || null != loadTask || null == BundleLoader.Ins
			) //有可能出现，因为通过depend的回调有可能先执行到这，然后在StartLoaded执行到这
				return;
