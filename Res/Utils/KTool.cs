...
namespace AD
{
    ...
    public static class KTool
    {
        ...
        public static bool IsBetweenWave(string waveNumberStr, int testNumber)
        {
            ...
            if (strs.Length == 1)
            {
                return strs[0].ToInt() == testNumber;
            }
            var min = strs[0].ToInt();
            var max = strs[1].ToInt();
        ...
        public static bool IsContains(string numberStr, string testValue, char sp = ',')
        {
            ...
            return strs.Contains(testValue);
        }
        ...
        public static Shader FindShader(string shaderName)
        {
            ...
            if (!CacheShaders.TryGetValue(shaderName, out shader))
            {
                ...
                if (shader == null)
                    Debugger.Error("缺少Shader：{0}  ， 检查Graphics Settings的预置shader", shaderName);
            }
            ...
            for (int i = 0; i < tArray.Length; i++)
            {
                ...
                switch (typeCode)
                {
                    ...
                    case TypeCode.UInt64:
                        ...
                        break;
                    default:
                        Debugger.Error("Unsupport Type {0} in StrBytesToArray(), You can custom this.", typeCode);
        ...
        public static bool TestCron(string cron)
        {
            ...
            Debugger.DoLog("Cron:{0}, now: {1}, next: {2}", cron, now, next);
            var span = next - now;
            Debugger.DoLog(span.TotalMinutes.ToString());
        ...
        */
        ...
        public static string FormatArgs(string source, object[] args)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                {
                    Debugger.Error($"not find argument index: \"{paramKey}\" in array: {args}");
                }
            }
        ...
        public static T GetChildComponent<T>(Transform findTrans, string uri, bool isLog = true) where T : Component
        {
            ...
            {
                if (isLog)
                    Debugger.Error("Get Child Error: " + uri);
                return default(T);
            }
        ...
        public static GameObject GetGameObject(string name, Transform findTrans, bool isLog = true)
        {
            ...
            if (obj == null)
            {
                Debugger.Error("Find GemeObject Error: " + name);
                return null;
            }
