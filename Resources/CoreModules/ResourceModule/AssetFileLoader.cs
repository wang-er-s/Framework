    ...
    public class AssetFileLoader : AbstractResourceLoader
    {
        ...
        private IEnumerator _Init(string path, LoaderMode loaderMode)
        {
            ...
            if (Configs.IsUseResources)
            {
                ...
                if (getAsset == null)
                {
                    Debugger.Error("Asset is NULL(from Resources Folder): {0}", path);
                }
                OnFinish(getAsset);
            }
			if (Configs.IsEditor && Configs.IsEditorLoadAsset) 
			{
				...
				if (path.EndsWith(".unity"))
				{
					...
					Debugger.Error($"Load scene from Build Settings: {path}");
				}
				else 
				{
                    var paths = UnityEditor.AssetDatabase.FindAssets("Cube", new[] { Path.GetDirectoryName(path) });
                    if (paths.Length <= 0)
                    {
                        Debugger.Exception(
                            new Exception($"Asset is NULL(from {Configs.EditorResourcesDir} Folder): {path}"));
                    }
                    path = UnityEditor.AssetDatabase.GUIDToAssetPath(paths[0]);
                    getAsset = UnityEditor.AssetDatabase.LoadAssetAtPath(path , typeof(UnityEngine.Object));
                }
#else
				Debugger.Error("`IsEditorLoadAsset` is Unity Editor only");
            ...
            else
            {
                ...
                {
                    Debugger.Error("[AssetFileLoader]Load BundleLoader Failed(Error) when Finished: {0}", path);
                    _bundleLoader.Release();
                    OnFinish(null);
                    yield break;
                }
                ...
                if (getAsset == null)
                {
                    Debugger.Error("Asset is NULL: {0}", path);
                }
            ...
            if (getAsset != null && Configs.IsLoadBundle)
            {
...
//                    Object.DestroyImmediate(ResultObject as Object, true);
                ...
                //if (!bRemove)
                //{
                //    Debugger.Warning("[DisposeTheCache]Remove Fail(可能有两个未完成的，同时来到这) : {0}", Url);
                //}
            }
            //else
            //{
