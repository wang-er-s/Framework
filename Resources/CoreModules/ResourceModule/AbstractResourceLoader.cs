...
namespace AD
{
    ...
    public abstract class AbstractResourceLoader : IAsyncObject
    {
            ...
            params object[] initArgs) where T : AbstractResourceLoader, new()
        {
            ...
            {
                Debugger.Error("[{0}:AutoNew]url为空", typeof(T));
            }
        ...
        internal static void DoGarbageCollect()
        {
            ...
            {
                Debugger.Error("[DoGarbageCollect]CacheLoaderToRemoveFromUnUsed muse be empty!!");
            }
        }
            ...
            if (callback != null)
            {
                if (IsCompleted)
                {
                    if (ResultObject == null)
                        Debugger.Warning("Null ResultAsset {0}", Url);
...
//                IsBeenReleaseNow = true;
        ...
        public virtual void Release()
        {
            ...
            {
                Debugger.Warning("[{0}]Too many dipose! {1}, Count: {2}", GetType().Name, this.Url, RefCount);
            }
            ...
            if (RefCount <= 0)
            {
                ...
                if (Debug.isDebugBuild)
                {
                    ...
                    {
                        Debugger.Error(
                            $"[{GetType()}]RefCount< 0, {GetType().Name} : {Url}, NowRefCount: {RefCount}, Will be fix to 0");
                    ...
                    {
                        Debugger.Error($"[{GetType()}]UnUsesLoader exists: {this}");
                    }
                }
        ...
        private void Dispose()
        {
            ...
            if (!IsForceNew)
            {
                ...
                //if (Url != null) // TODO: 以后去掉
                {
                    ...
                    if (!bRemove)
                    {
                        Debugger.Warning("[{0}:Dispose]No Url: {1}, Cur RefCount: {2}", type.Name, Url, RefCount);
                    }
                }
            }
        ...
        public virtual void ForceDispose()
        {
            ...
            if (RefCount != 1)
            {
                Debugger.Warning("[ForceDisose]Use force dispose to dispose loader, recommend this loader RefCount == 1");
            }
            Dispose();
        }
